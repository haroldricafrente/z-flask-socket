...
// Wi-Fi credentials for Shiitake project
const char* ssid = "OT7";
const char* password = "dIk0aLAm";

// Select camera model (same model for Shiitake project)
...
// Stream handler for Shiitake project
...
// Capture image handler for Shiitake project
static esp_err_t capture_image_handler(httpd_req_t *req) {
  camera_fb_t * fb = NULL;
  esp_err_t res = ESP_OK;

  // In your capture_image_handler function on ESP32 for Shiitake
...
// Resolution change handler for Shiitake project
...
void setup() {
  ...
  startCameraServer();
  Serial.println("Shiitake Mushroom Camera Ready! Go to: http://192.168.100.193:81/stream");
}
