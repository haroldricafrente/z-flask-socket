...
// Humidity thresholds
const int HUMIDITY_HIGH_THRESHOLD = 75;  // Adjust if needed for milky mushroom
const int HUMIDITY_LOW_THRESHOLD = 80;
...
void setup() {
  ...
  // Time setup
  timeClient.begin();
  timeClient.update();  // Force initial sync
...
void controlFan() {
  if (fanState) {
    ...
    if (currentMillis - previousFanMillis >= fanOnDuration) {
      digitalWrite(FAN_RELAY_PIN, HIGH);  // Turn OFF (active-low relay)
      fanState = false;
      previousFanMillis = currentMillis;  // Reset timer for OFF duration
      Serial.println("Fan OFF");
    }
  } else {
    // Fan is OFF, check if it's time to turn ON
    if (currentMillis - previousFanMillis >= fanOffDuration) {
      digitalWrite(FAN_RELAY_PIN, LOW);  // Turn ON (active-low relay)
      fanState = true;
      previousFanMillis = currentMillis;  // Reset timer for ON duration
